...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	for item, item_dict in item_details.items():
		...
		row = [details.name, details.item_name, details.custom_new, details.custom_used, details.description, details.item_group, details.brand]
...
def get_columns(filters: Filters) -> List[Dict]:
	...
	columns = [
		...
		{
			"label": _("New"), 
			"fieldname": "custom_new", 
			"fieldtype": "Check", 
			"width": 70
		},
		{
			"label": _("Used"), 
			"fieldname": "custom_used", 
			"fieldtype": "Check", 
			"width": 70
		},

frappe.listview_settings['Delivery Note'] = {
	add_fields: ["customer", "customer_name", "base_grand_total", "per_installed", "per_billed",
		"transporter_name", "grand_total", "is_return", "status", "currency", "custom_add_to_transit", "custom_lost_wastage"],
	get_indicator: function(doc) {
		...
		} else if (flt(doc.per_billed, 2) < 100 && !doc.custom_add_to_transit) {
			return [__("To Bill"), "orange", "per_billed,<,100"];
        } else if (flt(doc.per_billed, 2) < 100 && doc.custom_add_to_transit) {
			let per_billed = doc.per_billed + doc.custom_lost_wastage;
			if(per_billed===100){
				return [__("Completed"), "green", "per_billed,=,100"];
			}else{
				return [__("To Receive"), "orange", "per_billed,<,100"];
			}
